...
class SchemaGetDeviceAppDeploys(Schema):
    """Schema for GetDeviceAppDeploys API.
...
class GetDeviceAppDeploys(ConsoleAccessBaseClass):
    ...
    def get_device_app_deploys(self, app_name, version_number):
        ...
        try:
            ...
            # Validate schema
            _local_params = SchemaGetDeviceAppDeploys().load(_local_params)
            ...
            with aitrios_console_rest_client_sdk_primitive.ApiClient(
                ...
                try:
                    ...
                    _return_get_device_app_deploys = self.on_key_error_response(__name__, key_err)

                except aitrios_console_rest_client_sdk_primitive.ApiTypeError as type_err:
                    _return_get_device_app_deploys = self.on_type_error_response(__name__, type_err)

                except aitrios_console_rest_client_sdk_primitive.ApiValueError as val_err:
                    _return_get_device_app_deploys = self.on_value_error_response(__name__, val_err)

                except aitrios_console_rest_client_sdk_primitive.ApiAttributeError as attr_err:
                    _return_get_device_app_deploys = self.on_attribute_error_response(
                        __name__, attr_err
                    )

                except aitrios_console_rest_client_sdk_primitive.ApiException as exception:
                    _return_get_device_app_deploys = self.on_http_error_response(
                        __name__, exception
                    )

                except Exception as exception:
                    _return_get_device_app_deploys = self.on_generic_error_response(
                        __name__, exception
                    )

        except ValidationError as err:
            _return_get_device_app_deploys = self.on_validation_error_response(__name__, err)

        return _return_get_device_app_deploys
