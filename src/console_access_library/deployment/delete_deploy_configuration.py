...
class SchemaDeleteDeployConfiguration(Schema):
    """Schema for DeleteDeployConfiguration API.
...
class DeleteDeployConfiguration(ConsoleAccessBaseClass):
    """This class implements DeleteDeployConfiguration API.
    ...
    def delete_deploy_configuration(self, config_id: str):
        """Delete deployment config information

        Args:
            config_id (str, required): The Config ID.
        ...
        try:
            ...
            # Validate schema
            _local_params = SchemaDeleteDeployConfiguration().load(_local_params)
            ...
            with aitrios_console_rest_client_sdk_primitive.ApiClient(
                ...
                try:
                    _return_delete_deploy_configuration = (
                        deploy_api_instance.delete_deploy_configuration(path_params=_local_params)
                    )
                    return _return_delete_deploy_configuration.body

                except aitrios_console_rest_client_sdk_primitive.ApiKeyError as key_err:
                    _return_delete_deploy_configuration = self.on_key_error_response(
                        __name__, key_err
                    )

                except aitrios_console_rest_client_sdk_primitive.ApiTypeError as type_err:
                    _return_delete_deploy_configuration = self.on_type_error_response(
                        __name__, type_err
                    )

                except aitrios_console_rest_client_sdk_primitive.ApiValueError as val_err:
                    _return_delete_deploy_configuration = self.on_value_error_response(
                        __name__, val_err
                    )

                except aitrios_console_rest_client_sdk_primitive.ApiAttributeError as attr_err:
                    _return_delete_deploy_configuration = self.on_attribute_error_response(
                        __name__, attr_err
                    )

                except aitrios_console_rest_client_sdk_primitive.ApiException as exception:
                    _return_delete_deploy_configuration = self.on_http_error_response(
                        __name__, exception
                    )

                except Exception as exception:
                    _return_delete_deploy_configuration = self.on_generic_error_response(
                        __name__, exception
                    )

        except ValidationError as err:
            _return_delete_deploy_configuration = self.on_validation_error_response(__name__, err)

        return _return_delete_deploy_configuration
