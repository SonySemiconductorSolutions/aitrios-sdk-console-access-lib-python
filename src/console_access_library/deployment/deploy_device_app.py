...
class SchemaDeployDeviceApp(Schema):
    """Schema for DeployDeviceApp API.
...
class DeployDeviceApp(ConsoleAccessBaseClass):
    """This class implements DeployDeviceApp API.
    ...
    def deploy_device_app(
        ...
        Example:
            .. code-block:: python
                ...
                response = deployment_obj.deploy_device_app(
                    app_name,
                    version_number,
                    device_ids,
                    deploy_parameter,
        ...
        try:
            ...
            with aitrios_console_rest_client_sdk_primitive.ApiClient(
                ...
                try:
                    ...
                    _return_deploy_device_app = self.on_key_error_response(__name__, key_err)

                except aitrios_console_rest_client_sdk_primitive.ApiTypeError as type_err:
                    _return_deploy_device_app = self.on_type_error_response(__name__, type_err)

                except aitrios_console_rest_client_sdk_primitive.ApiValueError as val_err:
                    _return_deploy_device_app = self.on_value_error_response(__name__, val_err)

                except aitrios_console_rest_client_sdk_primitive.ApiAttributeError as attr_err:
                    _return_deploy_device_app = self.on_attribute_error_response(__name__, attr_err)

                except aitrios_console_rest_client_sdk_primitive.ApiException as exception:
                    _return_deploy_device_app = self.on_http_error_response(__name__, exception)

                except Exception as exception:
                    _return_deploy_device_app = self.on_generic_error_response(__name__, exception)

        except ValidationError as err:
            _return_deploy_device_app = self.on_validation_error_response(__name__, err)

        return _return_deploy_device_app
