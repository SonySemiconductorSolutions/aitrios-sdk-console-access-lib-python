...
class SchemaDeployByConfiguration(Schema):
    """Schema for API to deployment by configuration.
    ...
    def validate(self, data, **kwargs):
        ...
        if "replace_model_id" in data and (
            data["replace_model_id"] is None or str(data["replace_model_id"]).strip() == ""
        ):
...
class DeployByConfiguration(ConsoleAccessBaseClass):
    ...
    def deploy_by_configuration(
        ...
        Example:
            .. code-block:: python
                ...
                response = deployment_obj.deploy_by_configuration(
                    config_id,
                    device_ids,
                    replace_model_id,
                    comment,
        ...
        try:
            ...
            if "replace_model_id" in _local_params and _local_params["replace_model_id"] is None:
                del _local_params["replace_model_id"]

            if "comment" in _local_params and _local_params["comment"] is None:
                del _local_params["comment"]

            # Validate Schema
            _local_params = SchemaDeployByConfiguration().load(_local_params)

            _path_params = {
                "config_id": _local_params["config_id"],
            }
            _query_params = _local_params
            del _query_params["config_id"]
            ...
            with aitrios_console_rest_client_sdk_primitive.ApiClient(
                ...
                try:
                    _return_deploy_by_configuration = (
                        deploy_by_configuration_api_instance.deploy_by_configuration(
                            path_params=_path_params, query_params=_query_params
                        )
                    )
                    return _return_deploy_by_configuration.body

                except aitrios_console_rest_client_sdk_primitive.ApiKeyError as key_err:
                    _return_deploy_by_configuration = self.on_key_error_response(__name__, key_err)

                except aitrios_console_rest_client_sdk_primitive.ApiTypeError as type_err:
                    _return_deploy_by_configuration = self.on_type_error_response(
                        __name__, type_err
                    )

                except aitrios_console_rest_client_sdk_primitive.ApiValueError as val_err:
                    _return_deploy_by_configuration = self.on_value_error_response(
                        __name__, val_err
                    )

                except aitrios_console_rest_client_sdk_primitive.ApiAttributeError as attr_err:
                    _return_deploy_by_configuration = self.on_attribute_error_response(
                        __name__, attr_err
                    )

                except aitrios_console_rest_client_sdk_primitive.ApiException as exception:
                    _return_deploy_by_configuration = self.on_http_error_response(
                        __name__, exception
                    )

                except Exception as exception:
                    _return_deploy_by_configuration = self.on_generic_error_response(
                        __name__, exception
                    )

        except ValidationError as err:
            _return_deploy_by_configuration = self.on_validation_error_response(__name__, err)

        return _return_deploy_by_configuration
