...
class SchemaStartUploadInferenceResult(Schema):
    """Schema for API for StartUploadInferenceResult.
...
class StartUploadInferenceResult(ConsoleAccessBaseClass):
    """This class implements API to execute instructions for starting acquisition of \
        inference result metadata (Output Tensor) and image (Input Tensor) to the specified device.
    ...
    def start_upload_inference_result(self, device_id: str):
        ...
        Returns:
            dict:
                - **Success Response** : Dictionary with below key and value pairs.
                    ...
                    - ``result`` (str) : Result
                    - ``outputSubDirectory`` (str) : Input Image storage path,
                            UploadMethod:Blob Storage only
        ...
        Example:
            .. code-block:: python
                ...
                response = console_sdk_client_obj.device_management.start_upload_inference_result(
                    device_id)
                pprint(response)
        """
